...
```{r}
Boxplot_pregnancies <- data |> 
  ggplot(Pregnancies, aes(x = 1, y = data[[col]])) +
  geom_boxplot() +
  theme_minimal() +
...
Change age_onset to age ranges.
Change Outcome variable to Diabetic (1) and Non-diabtic (0).
...
Goal: predict diabetic or non-diabetic.
PCA : with the outcome variable.
